...
# --- NEW: Threshold for fuzzy matching ---
# A keyword from the user must have at least this score (out of 100) to be considered a match.
FUZZY_MATCH_THRESHOLD = 85

# Define stop words again for processing user input
STOP_WORDS = {
    'en': {'a', 'an', 'and', 'the', 'is', 'in', 'it', 'i', 'to', 'for', 'of', 'how', 'do', 'can', 'what', 'why', 'my', 'this', 'app'},
    'hi': {'एक', 'में', 'है', 'की', 'मैं', 'यह', 'से', 'क्या', 'कैसे', 'क्यों', 'मेरा', 'मेरे', 'इस', 'और', 'कर'},
    'id': {'di', 'ke', 'dari', 'dan', 'ini', 'itu', 'saya', 'untuk', 'bagaimana', 'apa', 'kenapa', 'bisa', 'aplikasi', 'yang'}
}
GREETINGS = {"en": {"hi", "hello", "hey"}, "hi": {"नमस्ते", "हाय", "हेलो"}, "id": {"hai", "halo"}}
GREETING_RESPONSES = {"en": "Hello! How can I assist you?", "hi": "नमस्ते! मैं कैसे मदद कर सकता हूँ?", "id": "Hai! Bagaimana saya bisa membantu?"}
...
def normalize_and_tokenize(text, lang):
    """Normalizes user input for searching."""
    if not isinstance(text, str): return []
    text = text.lower()
    text = re.sub(r"[^\w\s\u0900-\u097F]", "", text)
    words = text.split()
    return [word for word in words if word not in STOP_WORDS.get(lang, set())]

def find_best_match_by_keyword(user_query, lang):
    """
    Finds the best match using a FUZZY keyword scoring system.
    This can handle spelling mistakes.
    """
    user_keywords = normalize_and_tokenize(user_query, lang)
    if not user_keywords:
        return "I'm sorry, I didn’t understand that. Please rephrase your question."

    best_score = 0
    best_answer = "I'm sorry, I didn’t understand that. Please rephrase your question."

    # Iterate through all FAQ items for the detected language
    for faq_item in search_data.get(lang, []):
        faq_keywords = faq_item['keywords']
        current_item_score = 0

        # For each keyword from the user, find its best fuzzy match in the FAQ's keywords
        for user_word in user_keywords:
            # process.extractOne returns the best match and its score (0-100)
            best_match = process.extractOne(user_word, faq_keywords, scorer=fuzz.ratio)
            
            if best_match and best_match[1] >= FUZZY_MATCH_THRESHOLD:
                # Add the confidence score of the match to the total score for this FAQ item
                current_item_score += best_match[1]
        
        if current_item_score > best_score:
            best_score = current_item_score
            best_answer = faq_item['answer']

    # Require a minimum score to be considered a valid match
    # The score is now based on confidence, so a threshold around 80-90 is reasonable
    if best_score > 80:
        logger.info(f"Found fuzzy match for query '{user_query}' with score {best_score}")
        return best_answer
    else:
        logger.warning(f"No good fuzzy match found for query '{user_query}'")
        return best_answer

# --- HTML FRONTEND & FLASK ROUTES ---
...
def chat():
    ...
    try:
        ...
        lang = 'en'
    logger.info(f"Detected language: {lang} for query: '{user_input}'")
    normalized_words = set(normalize_and_tokenize(user_input, lang))
    if any(greet in normalized_words for greet in GREETINGS.get(lang, set())):
         return jsonify({"response": GREETING_RESPONSES.get(lang, "Hello!")})
    response = find_best_match_by_keyword(user_input, lang)
    return jsonify({"response": response})

if __name__ == '__main__':
    logger.info("Starting Keyword-Based Flask server with Fuzzy Matching...")
